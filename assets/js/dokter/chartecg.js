...
if (url.includes('record')) {
	...
	$(document).ready(function () {
		var loopRange = 1;
		console.log(base + "data/getDataRekaman/" + url[6] + "/" + url[7]);
		$.ajax({
			url: base + "data/getDataRekaman/" + url[6] + "/" + url[7],
			type: 'POST',
			dataType: 'json',
		...
		$.ajax({
			...
			success: function (response) {
				response.forEach(function (row, i) {
					xValue = parseInt(row.annotation);
					yValue = parseInt(row.ecg);
					temp.push({
						x: xValue,
						y: yValue,
						marker: {
